...
# now contsturct and deliver that as a fall back/default certificate for haproxy

mkdir -p /opt/local/etc/certs/

cat /tmp/key.pem /tmp/cert.pem > /opt/local/etc/certs/aaaaaaaaa-default-wildcard.cert
